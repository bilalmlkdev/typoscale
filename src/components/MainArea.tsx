...
export function MainArea({ onOpenNote }: MainAreaProps) {
  ...
  return (
    ...
    >
      <div
        className={`border-b px-4 sm:px-6 py-4 flex flex-col gap-4 flex-shrink-0 transition-colors duration-300 ${
          darkMode
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-2.5">
            <svg
              className={`w-4 h-4 hidden xs:block ${darkMode ? "text-stone-500" : "text-stone-400"}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            <div className="flex items-center gap-1.5 text-sm sm:text-base">
              <span
                className={`font-medium tracking-tight ${darkMode ? "text-stone-400" : "text-stone-500"}`}
              >
                bilalmlkdev
              </span>
              <span
              ...
              <span
                className={`font-bold tracking-tight ${darkMode ? "text-stone-100" : "text-stone-900"}`}
              >
                typoscale
              </span>
              <span
                className={`text-[10px] ml-1.5 px-2 py-0.5 rounded-full font-mono font-medium border ${
                  darkMode
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            <div
              className={`flex rounded-lg p-0.5 border ${darkMode ? "bg-stone-950 border-stone-800" : "bg-stone-50 border-stone-200"}`}
            >
              {(["preview", "responsive", "fluid", "tokens"] as const).map(
                (tab: ActiveTabType) => (
                  <button
                    key={tab}
            ...
            >
              <span
                className={`px-2 py-1.5 font-bold transition-colors ${
                  darkMode
                    ? "bg-stone-900 text-stone-400 border-r border-stone-800"
                    : "bg-stone-200 text-stone-600 border-r border-stone-300"
                }`}
              >
                $ core-sh
              </span>
              <span
                className={`px-2.5 py-1.5 font-medium transition-all flex items-center gap-1.5 ${
                  darkMode
        ...
        <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-dashed border-stone-200 dark:border-stone-800/60">
          ...
          <Stat
            label="scale-ratio"
            value={effectiveRatio.toFixed(3)}
            darkMode={darkMode}
          />
          <Stat
            label="total-steps"
...
function Stat({
  ...
  return (
    <div
      className={`inline-flex items-center rounded-md text-[11px] font-mono border overflow-hidden transition-all shadow-sm ${
        darkMode
    ...
    >
      <span
        className={`px-2 py-0.5 font-medium border-r ${
          darkMode
            ? "bg-stone-900 border-stone-800 text-stone-400"
            : "bg-stone-200/60 border-stone-200 text-stone-500"
        }`}
      >
        {label}
      </span>
      <span
        className={`px-2 py-0.5 font-bold ${darkMode ? "text-amber-400" : "text-amber-400"}`}
      >
        {value}
      </span>
    </div>
