...
export function WelcomeNote({ isOpen, onClose, darkMode }: WelcomeNoteProps) {
  ...
  const base = d
    ? "bg-[#0f0f0f] text-stone-200 border-stone-800"
    : "bg-white text-[#111] border-stone-200";
  ...
  const features = [
    {
      icon: <Sliders size={13} strokeWidth={1.5} />,
      name: "Modular scale ratios",
      desc: "Minor Second to Golden Ratio, or any custom value.",
    },
    {
      icon: <Code2 size={13} strokeWidth={1.5} />,
      name: "CSS, Tailwind & JSON export",
      desc: "One-click copy for all token formats.",
    },
    {
      icon: <Type size={13} strokeWidth={1.5} />,
      name: "Live font pairing",
      desc: "Pair any Google Font. Preview updates instantly.",
    },
    {
      icon: <Smartphone size={13} strokeWidth={1.5} />,
      name: "Responsive preview",
      desc: "Drag to resize across breakpoints.",
    },
    {
      icon: <Heart size={13} strokeWidth={1.5} />,
      name: "Free forever",
      desc: "No account, no limits, open source.",
    },
  ];
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-col md:flex-row">
          ...
          >
            <div
              className={`inline-flex items-center gap-1.5 text-[10px] px-2 py-0.5 rounded-full border w-fit
            ...
            <div>
              <h1 className="text-xl font-semibold tracking-tight leading-tight mb-0.5">
                TypoScale
              </h1>
              <p className={`text-[11px] ${muted}`}>
                Type scale generator for design systems
              </p>
            </div>

            <p className={`text-[11.5px] leading-relaxed ${muted}`}>
              Create harmonious typography systems in seconds. Generate CSS
              tokens, Tailwind config, and design variables — ready to drop in.
            </p>
            ...
            <div className={`rounded border p-2 ${subtle}`}>
              <span
                className={`block text-[9px] tracking-widest mb-1 ${muted}`}
              >
                QUICK START
              </span>
            ...
            <div className="flex gap-1.5 flex-wrap">
              ...
              <a
                href="https://github.com/bilalmlkdev/typoscale/issues"
                target="_blank"
                rel="noopener noreferrer"
                className={btnGhost}
              >
            ...
            <p className={`text-[10px] mt-auto ${muted}`}>
          ...
          <div className="flex-1 p-5 flex flex-col">
            ...
            <div className="flex flex-col">
              {features.map((f, i) => (
                <div
                  key={f.name}
                  className={`flex items-start gap-2 py-1.5 ${i < features.length - 1 ? `border-b ${divider}` : ""}`}
                >
                  <span className={`mt-0.5 flex-shrink-0 ${muted}`}>
                    {f.icon}
                  </span>
                  <div>
                    <span className="block text-[11.5px] font-medium leading-snug">
                      {f.name}
                    </span>
                    <span className={`text-[10.5px] leading-relaxed ${muted}`}>
                      {f.desc}
                    </span>
                  </div>
                </div>
            ...
            <p className={`text-[9px] tracking-widest mb-2 ${muted}`}>
              CONTRIBUTING
            </p>
            <p className={`text-[11px] leading-relaxed mb-3 ${muted}`}>
              Open an issue or PR on GitHub. Please read{" "}
              <code className={`text-[10.5px] px-1 py-0.5 rounded ${subtle}`}>
                CONTRIBUTING.md
              </code>{" "}
              first.
            </p>
            ...
            <div className="flex flex-wrap gap-1.5 mt-auto">
              {["good first issue", "help wanted", "TypeScript", "React"].map(
                (label) => (
